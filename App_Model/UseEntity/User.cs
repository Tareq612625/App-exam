...
namespace App_Model.UseEntity
{
    public class User
    {
        ...
        [Required(ErrorMessage = "{0} is required")]
        [StringLength(maximumLength: 200, ErrorMessage = "{0} length is between {2} and {1}", MinimumLength = 3)]
        ...
        public string EncryptedPassword
        {
            get
            {
                return EncryptionHelper.Encrypt(UserPassword.ToString());
            }
            set
            {
                UserId = EncryptionHelper.Decrypt(value);
